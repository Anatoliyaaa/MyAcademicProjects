...
namespace ЛР_8
{
    struct Matrix4
    {
        ...
        public void Rotation(Vector3f Rot)
        {
            ...
            matX.Colum3.Z = (float)Math.Cos(Rot.X);

            //MatY
            matY.Colum2.Y = 1;
            matY.Colum4.W = 1;

            matY.Colum1.X = (float)Math.Cos(Rot.Y);
            matY.Colum1.Z = -(float)Math.Sin(Rot.Y);

            matY.Colum3.X = (float)Math.Sin(Rot.Y);
            matY.Colum3.Z = (float)Math.Cos(Rot.Y);
            ...
            matZ.Colum1.X = (float)Math.Cos(Rot.Z);
            matZ.Colum1.Y = -(float)Math.Sin(Rot.Z);

            matZ.Colum2.X = (float)Math.Sin(Rot.Z);
            matZ.Colum2.Y = (float)Math.Cos(Rot.Z);
